...
$(document).ready(() => {
  ...
  if (map2) {
    if ("undefined" !== typeof ymaps) ymaps.ready((() => initMainMap()));
    else console.warn("Yandex Maps API не загружено для карты #map2");
    function initMainMap() {
      try {
        ...
        const placemark2 = new ymaps.Placemark([55.760593, 37.502981], {}, {
          iconLayout: "default#image",
        ...
        console.error("Ошибка при инициализации карты #map2:", error);
      }
    }
  }
  ...
  document.querySelectorAll('input[type="radio"].radio__input').forEach(radio => {
    ...
    // Если есть привязка к блоку — сохраняем
    if (targetClass) {
      // Проверяем, отмечена ли радио сейчас
      if (radio.checked) {
      ...
      radio.addEventListener('change', () => {
        // Удаляем классы checked и _active у всех
        ...
        // Добавляем классы только текущему элементу
        if (radio.checked) {
          label.classList.add('checked');
          document.querySelector(targetClass)?.classList.add('_active');
        }
    ...
    // Добавляем класс checked, если радио уже отмечена
    if (radio.checked) {
      radio.closest('.checkbox-text').classList.add('checked');
    }

    // Обновляем класс при изменении выбора
    radio.addEventListener('change', () => {
      // Удаляем класс checked у всех в этом списке
      document.querySelectorAll('.checkbox-text').forEach(el => {
        el.classList.remove('checked');
      });

      // Добавляем класс текущей выбранной
      if (radio.checked) {
        radio.closest('.checkbox-text').classList.add('checked');
      }
    });
  ...
  AW.initSliderTable = function ($el) {
    const $wrapper = $el.closest('[data-swiper-wrapper="swiper-tables"]');
  ...
  $('[data-swiper="swiper-tables"]').each(function () {
    AW.initSliderTable($(this));
  });
  ...
  // Функция для обработки всех блоков с табами
  function initTabs() {
    $('.block-tabs').each(function () {
      const tabsContainer = $(this);
      const currentActiveTabId = tabsContainer.find('.heading-complex__title._tab-active').first().attr('data-id') ||
        tabsContainer.find('.heading-complex__title').first().attr('data-id');

      // Сохраняем последнюю активную вкладку
      tabsContainer.data('last-active-tab', currentActiveTabId);

      // Активируем текущую вкладку
      switchTab(tabsContainer, currentActiveTabId);

      // Обработчик кликов
      tabsContainer.on('click', '.heading-complex__title', function (e) {
        e.preventDefault();
        e.stopPropagation();
      ...
      // Защита от случайного снятия класса _tab-active
      const observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
          if (mutation.attributeName === 'class') {
            const activeTab = tabsContainer.find('.heading-complex__title._tab-active');
            if (!activeTab.length) {
              // Если активная вкладка пропала - восстанавливаем
              const lastActiveTabId = tabsContainer.data('last-active-tab');
              if (lastActiveTabId) {
                switchTab(tabsContainer, lastActiveTabId);
              }
            }
          }
        });
      ...
      // Наблюдаем за изменениями классов у кнопок вкладок
      tabsContainer.find('.heading-complex__title').each(function () {
  ...
  function switchTab(tabsContainer, tabId) {
    ...
    tabsContainer.data('last-active-tab', tabId);

    // Удаляем классы у всех кнопок и тел вкладок в текущем контейнере
    tabsContainer.find('.heading-complex__title._tab-active').removeClass('_tab-active');
    tabsContainer.find('.tabs__body._tab-active').removeClass('_tab-active');

    // Добавляем классы активным элементам
    const activeButton = tabsContainer.find(`.heading-complex__title[data-id="${tabId}"]`);
    const activeBody = tabsContainer.find(`.tabs__body[data-id="${tabId}"]`);

    if (activeButton.length) activeButton.addClass('_tab-active');
    if (activeBody.length) activeBody.addClass('_tab-active');

    // Обновляем соответствующий слайдер
    const wrapperId = tabsContainer.find('[data-swiper-wrapper]').attr('data-swiper-wrapper');
  ...
  AW.initSliderPhoto = function ($el) {
    const $wrapper = $('[data-swiper-wrapper="photo"]');
    const $navNext = $wrapper.find('.swiper-nav_next');
    const $navPrev = $wrapper.find('.swiper-nav_prev');
    const $slides = $el.find('.swiper-slide');
    return new Swiper($el[0], {
  ...
  const priceInput = document.getElementById("feedback_price");
  const squareInput = document.getElementById("feedback_square");
  const openingsInput = document.getElementById("feedback_square2");
  const totalSquareOutput = document.getElementById("feedback_square3");
  const totalPriceOutput = document.getElementById("feedback_price2");

  if (priceInput && squareInput && openingsInput && totalSquareOutput && totalPriceOutput) {

    // Ищем цену на странице
    const cardPriceEl = document.querySelector(".card-product__price");
    let basePrice = 0;

    if (cardPriceEl) {
      // Берём текст и очищаем его от лишних символов
      const priceText = cardPriceEl.textContent.trim();
      basePrice = parseFloat(priceText.replace(/\s|\D+/g, '')) || 0;

      // Устанавливаем число в value (для скрытого использования)
      priceInput.value = basePrice;

      // Добавляем класс filled, если значение установлено
      if (basePrice > 0) {
    ...
    function updateTotals() {
      const square = parseFloat(squareInput.value.replace(/[^0-9.,]/g, '')) || 0;
      const openings = parseFloat(openingsInput.value.replace(/[^0-9.,]/g, '')) || 0;

      const totalSquare = Math.max(0, square - openings);
      const totalPrice = basePrice * totalSquare;

      totalSquareOutput.value = totalSquare > 0 ? `${totalSquare} кв.м.` : "0 кв.м.";
      totalPriceOutput.value = totalPrice > 0 ? `${formatNumber(Math.round(totalPrice))} ₽` : "0 ₽";
    }

    // Подписываемся на изменения
    squareInput.addEventListener("input", updateTotals);
    openingsInput.addEventListener("input", updateTotals);

    // Вызываем один раз при загрузке
    updateTotals();
  }

  // === Ваша проверка для .field-input1 ===
  const inputs = document.querySelectorAll('.field-input1');
  ...
  if (inputs.length > 0) {
    inputs.forEach(input => {
      ...
      // Инициализация filled при загрузке страницы
      if (input.value.trim() !== '') {
        input.classList.add('filled');
      }
    });
